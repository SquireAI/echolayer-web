<script lang="ts">
	...
	import { CREATE_ORG_PATH, DISCOVERY_HOME_PATH, INVALIDATE_SELECTED_ORG, ORGS_PATH, ORGS_SELECT_PATH } from "$lib/utils/paths";
            ...
            {:else}
                {#if hasOrgs}
                    <div class="flex flex-col gap-6 min-w-[400px]">
                        ...
                        {#each (orgs || []) as org}
                            <OrgItem organization={org} handleSelect={handleSelect} />                            
                        {/each}
                        </div>
                        <div class="flex flex-row w-full gap-6">
                        {#if $orgStore.entity && !$orgStore.loading}
                            <Button type="grey" class="px-8 py-3" full={true} handleClick={() => goto(previousPage)}>
                                Cancel
                            </Button>
                        {/if}
                            <Button type="primary" full={true} href={`${CREATE_ORG_PATH}`}>
                                Add new...
                            </Button>
                        </div>
                    </div>
