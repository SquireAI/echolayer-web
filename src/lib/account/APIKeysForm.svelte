...
<div class="flex flex-col gap-y-6 items-center w-full">
    ...
    <div class="flex w-full">
        <div class="flex flex-col sm:w-full lg:w-2/3">
            ...
            <p>Learn how to use the EchoLayer API in our <a href={`${NOTION_GETTING_STARTED_DOCS}`} target="_blank" class="underline">documentation.</a></p>
        </div>
    </div>
    <div class="flex flex-col w-full">
        ...
        {#if sortedAccessTokens.length > 0}
            {#each sortedAccessTokens as token (token.prefix)}
                <ReadOnlyAccessToken token={token} deleteTokenHandler={deleteAccessToken} />
            {/each}
        {/if}
        {#if !hasTokens}
