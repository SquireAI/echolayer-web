...
const derivedOriginStore: Readable<StoreOriginEntity> = derived(
	[originStore, ComponentStore],
	([$originStore, $ComponentStore]) => {
		if (!$originStore.entity || !$ComponentStore.entity) return initialValue;
		const derivedOrigin: BaseEntity | undefined = $ComponentStore.entity.find((component: ComponentEntity) => {
			return component.publicId === $originStore?.entity?.publicId;
		});
