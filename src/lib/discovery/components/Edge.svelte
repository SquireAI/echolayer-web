<script lang="ts">
	...
	function findConnectionName(
		...
		connections: AnchorConnectionData[]
	) {
...
<Edge let:path width={3} cornerRadius={4}>
	<defs>
		...
		<marker
			...
			viewBox="0 0 16 14"
			orient="auto"
			id={`${internalId}marker-end`}
		>
			<polyline
				class={`path__end-marker ${selected ? 'path__end-marker--selected' : ''}`}
				points="0,7 3,3.5 0,0"
				fill="none"
				stroke-width="1"
	...
	<path
		class={`edge__path ${selected ? 'edge__path--selected' : ''}`}
		d={path}
		marker-end={`url(#${internalId}marker-end)`}
	/>
	<span
		slot="label"
		class={`font-sans p-1 bg-white ${selected ? 'text-echolayer-blue-100' : 'text-neutral-500'}`}
		>{label}</span
	>
</Edge>
