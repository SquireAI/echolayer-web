...
<a href={`${ONBOARDING_PATH}${ORGS_SELECT_PATH}`}>
	<span
		class={`flex flex-row justify-between items-center px-1.5 rounded-lg bg-stone-500 bg-opacity-10 hover:bg-opacity-20 transition-all duration-300 ease-in-out`}
	>
		<span class="flex flex-row gap-2 items-center h-9">
			...
			{:else}
				<span class="block h-6 w-6 bg-stone-400 rounded-lg" />
